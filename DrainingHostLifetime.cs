...
public class DrainingHostLifetime : IHostLifetime, IDisposable
{
    ...
    {
        // Register POSIX signal handlers for SIGTERM (Kubernetes pod termination) and SIGINT (Ctrl+C / console termination)
    ...
    {
        // If a second signal (e.g. Ctrl+C pressed twice or second SIGTERM sent) arrives while draining is already in progress,
        ...
        if (_signalReceived)
        {
            ...
            context.Cancel = false; // Allow default OS termination to proceed immediately
        ...
        {
            _logger.LogInformation("OS Signal [{Signal}] received. Circuit draining is disabled via configuration (EnableDraining=false or DrainTimeoutSeconds<=0). Proceeding with immediate StopApplication().", context.Signal);
            _applicationLifetime.StopApplication();
            return;
        }
        ...
        // Immediately mark pod as shutting down so ASP.NET Core Readiness probe returns 503 Unhealthy.
        // Kubernetes/load balancers will remove this pod from routing endpoints within seconds so NO NEW users connect.
        ...
        // Start background draining task without blocking the OS signal callback thread
        Task.Run(async () => await DrainCircuitsAndShutDownAsync());
    }
    ...
    private async Task DrainCircuitsAndShutDownAsync()
    {
        ...
        while (stopwatch.Elapsed < maxDuration)
        {
            ...
            // If all circuits have naturally closed or disconnected, exit the drain loop
            ...
            // Log progress if count changed or every 15 seconds if verbose logging is enabled
